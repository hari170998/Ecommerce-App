...
const Register = () => {
    ...
    const handleSubmit=async(e) =>{
        e.preventDefault()
        try {
          const res=await axios.post("/api/v1/auth/register",{name,email,phone,password,address,answer});
          if(res.data.success){
  ...
  <div className="mb-3">
    <input type="text" value={answer} onChange={(e)=>setAnswer(e.target.value)} className="form-control" id="exampleInputEmail1" placeholder='What is your best friend name?'required />  
  </div>
